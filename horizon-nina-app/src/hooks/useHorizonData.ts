...
export const useHorizonData = () => {
  ...
  const [currentHorizon, setCurrentHorizon] = useState<HorizonData | null>(
    null
  );
  ...
  const addPointToCurrentHorizon = useCallback(
    (point: HorizonPoint) => {
  ...
  const updateHorizon = useCallback(
    async (id: string, updates: Partial<HorizonData>) => {
      try {
        setError(null);
        ...
        setError(
          err instanceof Error ? err.message : "Failed to update horizon"
        );
        throw err;
      }
    },
  ...
  const deleteHorizon = useCallback(
    async (id: string) => {
      try {
        ...
        setError(
          err instanceof Error ? err.message : "Failed to delete horizon"
        );
        throw err;
      }
    },
  ...
  const getHorizonById = useCallback(
    async (id: string): Promise<HorizonData | null> => {
      try {
  ...
  const getCoveragePercentage = useCallback((horizon: HorizonData): number => {
    ...
    const uniqueAzimuths = new Set(
      horizon.points.map((p) => Math.round(p.azimuth / 5) * 5)
    );
    return (uniqueAzimuths.size / 72) * 100; // 72 points for 5-degree resolution
  }, []);

  const isHorizonComplete = useCallback(
    (horizon: HorizonData): boolean => {
